...
namespace _3D_Tree_Generator
{
    ...
    class Tree : Mesh
    {
        ...
        /// <returns></returns>
        public static Tri[] GenerateBranch(out List<Leaf> leaves, //non optional parameter
                                                float radius = 1,  //optional parameters
                                                float height = 5, 
                                                int segments = 10, 
                                                int quality = 7,
                                                float darkness = 0.5f,
                                            ...
                                            )
        {
            ...
            trunkFunction = (trunkFunction == null) ? new Expression("x") : trunkFunction; //set to nefault values if null
            ...
            for (float i = 1; i < segments + 1; i++)
            {
                ...
                float currentRadius = i.CustInterp(0, segments, radius, topRadius, trunkFunction);


                float xOffset = xBendFunction.Eval(i) * xBendStrength;
                float zOffset = zBendFunction.Eval(i) * zBendStrength;
                float delta = (flare > 0 && currentHeight <= flareEnd) ? currentHeight.InterpRoot(0, flareEnd, flare * currentRadius * 0.5f, 0) : 0;

                Matrix4 matrix = Matrix4.CreateTranslation(xOffset, currentHeight, zOffset - delta); //define the translation to move the vertices generated to the correct height
                ...
                {            
                    ...
                    tris.AddRange(GenerateBranch(
                        out leaves,
                        currentRadius / 2f,
                        i.Lerp(0f, segments, height, 0) * (float)rnd.Range(0.7, 1.1),
                        segments,
                        quality,
                        depth: depth - 1,
                        darkness: 0.2f,
